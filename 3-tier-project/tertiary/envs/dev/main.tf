...
######################################################
# Secrets Manager for Backend
######################################################
resource "aws_secretsmanager_secret" "backend_db" {
  name        = "backend-db-secret"
  description = "Secret for backend RDS credentials"
}

resource "aws_secretsmanager_secret_version" "backend_db_version" {
...
module "loadbalancer" {
  ...
  vpc_id                  = module.vpc.vpc_id
  public_subnet_ids        = [ module.vpc.public_1, module.vpc.public_2]
...
module "ec2" {
  ...
  ami_frontend           = var.ami
  ami_backend            = var.ami
  ami_bastion            = var.ami
  instance_type          = "t2.micro"
  key_name               = var.key_name

  bastion_subnet_id      = module.vpc.public_1
  private_subnet_id_frontend = module.vpc.private_1
  private_subnet_id_backend = module.vpc.private_3
  sg_id                  = module.SG.private_sg_id
  bastion_sg_id          = module.SG.public_sg_id

  backend_secret_arn     = aws_secretsmanager_secret.backend_db.arn
  frontend_script_path   = "file1.sh"
  backend_script_path    = "file2.sh"
}
...
module "autoscaling" {
  ...
  source_frontend               = module.ec2.frontend_instance_id
  source_backend                = module.ec2.backend_instance_id
  key_name                      = var.key_name
  vpc_id                        = module.vpc.vpc_id
  private_subnet_ids_frontend = [module.vpc.private_1, module.vpc.private_2]
  private_subnet_ids_backend = [module.vpc.private_3, module.vpc.private_4  ]
